...
[Day23](/dev/articles/2025-css-advent-23) では、CSS が Declarative であること、その特徴を活かした Declarative な Design をすることで、本質に基づいた、Fault Tolerant な真の柔軟性を持ったデザインが可能になると述べました。

Declarative なマインドセットへのシフトに対して、デザインツールの現状はどうなっているのでしょうか？

## Design Tools

デザインツールと言っても様々ですが、今回論じたい内容はどのツールを取ってもほとんど変わらないため、ここでは Figma を例に話ます。

Adobe XD や Sketch といったデザインツールは以前からも存在していましたが、2015年に Beta 登場した Figma は、クラウドベースであり、プラグインなどのエコシステムをもち、コラボレーションに特化していた点で、デザインツールが開発にも活用されるようになったきっかけを作りました。

- [Five years of Figma](https://www.figma.com/five-years-of-figma/)

これらデザインツールは、Component-driven という設計思想というフロントエンド開発の文脈を、「情報・データを整理する」という、アプリケーションとしての機能として提供しています。
Figma においては、Layer や Component, Property, Swap Component などといった形で、Component-driven に即したデザインを可能にする機能をうまく実装していると思います。

「情報・データの整理」は、Web アプリケーションの得意とするところであり、デザインツールにおいては、これによって、デザインにおけるコンポーネントの再利用が容易になりました。

![デザインツールの登場](../../../../assets/images/design-tool.png)

しかしながら、デザインツールで作成したコンポーネントが、コンポーネントの実装時にそのまま参照可能かというと、そうではありません。
なぜならば、そこにはデザインツールとブラウザの異なる目的と制約があるからです。
