...
Typography に関するスタイルを設定する前に、筆者なりの軸となる考え方を整理しておきます。
...
適切に「設定する」、といってもユーザの設定を最大限に汲む姿勢で望むならば、極論は「設定しない」ことがフォントサイズに限っていえば最適な選択肢になり得ます。
...
ここでも、ユーザのフォントサイズ設定は仮定しません。
定数としての推奨サイズは保ったまま、 その伸縮係数として Viewport 単位を利用します。Fluid Typography も同様の仕組みで流動性を実現していましたが、その違いは「**定数値をユーザ設定の仮定から逆算しているか否か**」にあります。
...
これを物理サイズで考えた場合も、zoom でCSS ピクセル 1 つの物理サイズが 2 倍になってもCSS ピクセル数が半分になるので、物理的な大きさは実質変わりません。

このように `1vw` で指定した要素は zoom しても物理的な大きさが変わらないため、**画面幅を基準にしていては zoom に応じることができません**。
