...
SPA で JavaScript を中心としたコーディングが増える中、HTML/CSS という異なるコンテキストを分割して管理するよりも、JavaScript で一元管理した方がコーディングしやすく、可読性も高いという判断がなされたのです。

その結果、「望ましい UI の最終形を宣言し、その更新手順はフレームワークに委ねる」ことができるようになりました。

**「どのように（How）表示するか」ではなく「何を（What）表示するか」** のみを記述することをを UI 実装のパラダイムとして唱えたのが、宣言的 UI です。
...
また、CSS と JS の Co-location により、JavaScript の動的な値が自然に組み可能になった点も特筆すべき点でしょう。
